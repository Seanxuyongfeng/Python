...
#第一个卷积层
#输入通道3，输出通道32这个值值自己定义的，可以调整；卷积后图像尺寸不变，依然是32x32
...
#第2个卷积层
#输入通道32，输出通道64，这个也是指定的可以修改，卷积后图像尺寸不变，依然是16x16
...
#这64个8x8的图像实际上可以认为是从一副图片上提取的64个比较明显的特征
#这64个特征肯定重要程度不一样，也就是权重不一样
#经过全连接层之后训练得到合适的权重列表，这些权重应该会对应某一个类别
#也就是有这些特定的图片应该是某一类图片
